...
#define GPIO_PIN_MODE 0x04  //GPFSEL1
...
void dma_init_cbs()
{
  ...
  for (int index = DMA_PWM_PERIOD; index < 2 * DMA_PWM_PERIOD; index++) {
    ...
    cb->dest = PERI_BUS_BASE + PWM_BASE + PWM_FIFO;
    cb->tx_len = 4;
    cb->next_cb = ith_cb_bus_addr(((index + 1) % DMA_PWM_PERIOD) + DMA_PWM_PERIOD);  // on the last cb, this will point to DMA_PWM_PERIOD
  }
}
...
int main()
{
    ...
    uint8_t *dma_base_ptr = (uint8_t *)map_peripheral(DMA_BASE, PAGE_SIZE);
    dma_reg = (DMACtrlReg *)(dma_base_ptr + DMA_CHANNEL * 0x100);

    pwm_reg = (PWMCtrlReg *)map_peripheral(PWM_BASE, PWM_LEN);

    uint8_t *cm_base_ptr = (uint8_t *)map_peripheral(CM_BASE, CM_LEN);
    ...
    *(uint32_t *)dma_gpio_pin_on->virtual_addr = 1 << SERVO_PIN; 
    *(uint32_t *)dma_gpio_pin_off->virtual_addr = 1 << SERVO_PIN;

    gpio_mode_reg = (uint32_t *)map_peripheral(GPIO_BASE, 8);
    usleep(100);
    gpio_mode_reg[1] = 1 << ((SERVO_PIN % 10) * 3);  // set gpio pin to output
    munmap((void *) gpio_mode_reg, 8);  // once mode is set, this is no longer needed
